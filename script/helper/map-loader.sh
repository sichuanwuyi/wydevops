...
function _processProjectParamMapping() {
  ...
  for l_key in "${l_orderedKeyArray[@]}"; do
    ...
    if [ ! "${l_value}" ];then
      #该参数已经处理过了，直接跳过。
      info "map.loader.param.already.read" "${l_key}"
      continue
    fi
    ...
    for l_keyItem in ${l_keyItems[@]};do
      ...
      if [[ "${l_keyItem}" =~ ^(.*)=(.*)$ ]];then
        ...
        if [ "${l_paramValue}" == "null" ];then
          warn "map.loader.read.param.fail.from.files" "${l_shortFileNames}#${l_paramPath}"
          l_hasError="true"
          break
        fi
        ...
        if [[ "${l_subKeyItems[1]}" =~ ^\^ ]];then
          if [[ ! "${l_paramValue}" =~ ${l_subKeyItems[1]} ]];then
            #如果不匹配，则跳过该参数。
            warn "map.loader.param.value.not.match.regex" "${l_paramPath}#${l_paramValue}#${l_subKeyItems[1]}"
            l_hasError="true"
            break
          fi
        else
          ...
          if [ "${l_paramValue}" != "${l_subKeyItems[1]}" ];then
            #如果不匹配，则跳过该参数。
            warn "map.loader.param.value.not.equal" "${l_paramPath}#${l_paramValue}#${l_subKeyItems[1]}"
            l_hasError="true"
            break
          fi
        fi
        ...
        for l_valueItem in ${l_valueItems[@]};do
          ...
          if [[ "${l_tmpParamNames}" =~ ${l_subValueItems[0]}( |$) ]];then
            warn "map.loader.param.already.assigned" "${l_cicdConfigFile##*/}#${l_subValueItems[0]}"
            continue
          fi
          ...
          if [[ "${l_paramValue}" =~ \{\{.*\|([ ]*)default([ ]+)\"([^\"]+)\".*\}\} ]]; then
              ...
              warn "map.loader.extract.template.default.value" "${l_paramValue}"
          fi
          ...
          if [[ "${gDefaultRetVal}" =~ ^(\-1) ]];then
            warn "map.loader.update.param.fail" "${l_cicdConfigFile##*/}#${l_subValueItems[0]}"
            l_hasError="true"
            break
          else
            #记录已经赋过值的参数。
            eval "${l_mapName}[${l_subValueItems[0]}]=\"${l_paramValue}\""
            info "map.loader.update.param.success" "${l_cicdConfigFile##*/}#${l_subValueItems[0]}#${l_paramValue}"
            l_hasError="false"
          fi
...
function _readParamValueEx() {
  ...
  for l_sourceFile in ${l_sourceFiles[@]};do
    ...
    if [ ! -f "${l_sourceFile}" ];then
      warn "map.loader.file.not.exist.skip" "${l_sourceFile##*/}"
      continue
    fi
    ...
    if [ "${gDefaultRetVal}" != "null" ];then
      ...
      while (true);do
        if [[ "${l_paramValue}" =~ \$\{([^}]+)\} ]];then
            ...
            info "map.loader.found.placeholder.in.param.value" "${l_paramValue}#${l_varName}"
            _readParamValueEx "${l_buildPath}" "${l_targetFiles}" "${l_varName}" "true" "0"
            # 这里可以添加变量替换逻辑
            l_paramValue=${l_paramValue/\$\{$l_varName\}/${gDefaultRetVal}}
            info "map.loader.param.value.after.placeholder.replacement" "${l_paramValue}"
        else
            break;
        fi
      done
  ...
  if [[ "${l_found}" == "false" ]];then
    if [[ "${l_exitOnFailure}" == "true" ]];then
      error "map.loader.read.param.fail.ensure.exist" "${l_sourceFile##*/}#${l_paramPath}"
    else
      warn "map.loader.read.param.fail.ensure.exist" "${l_sourceFile##*/}#${l_paramPath}"
    fi
  fi
}
