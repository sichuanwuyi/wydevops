...
function combine(){
  ...
  local l_paramName
  local l_paramValue
  local l_unprocessedParams
  local l_unprocessedParamStr
  ...
  if [ "${l_srcParamPath}" ];then
    ...
    #首先读取源文件中第一个参数的前导空格数量。
    l_srcContent=$(grep -m 1 -oP "^([ ]*)[a-zA-Z_]+(.*):(.*)$" < "${l_srcYamlFile}")
    if [ "${l_srcContent}" ];then
      l_tmpSpaceNum=$(echo -e "${l_srcContent}" | grep -oP "^[ ]*" | grep -oP " " | wc -l)
      #再读取第一层的所有参数行信息。
      l_srcContent=$(grep -oP "^([ ]{${l_tmpSpaceNum}})[a-zA-Z_]+(.*):(.*)$" < "${l_srcYamlFile}")
    fi
   fi

  if [ "${l_srcContent}" ];then
    stringToArray "${l_srcContent}" "l_params"
    l_paramCount="${#l_params[@]}"

    l_unprocessedParams=()
    ((l_index = 0))
    for ((l_i = 0; l_i < l_paramCount; l_i++));do
      l_paramName="${l_params[${l_i}]%%:*}"
      l_paramName="${l_paramName// /}"
      l_paramValue="${l_params[${l_i}]#*:}"
      if [[ "${l_paramValue}" =~ ^([ ]*)\| ]];then
        readParam "${l_srcYamlFile}" "${l_paramName}"
        l_paramValue="${l_paramValue}\n${gDefaultRetVal}"
        updateParam "${l_targetYamlFile}" "${l_paramName}" "${l_paramValue}"
        if [[ "${gDefaultRetVal}" =~ ^(\-1) ]];then
          if [[ "${l_exitOnFailure}" && "${l_exitOnFailure}" == "true" ]];then
            error "更新目标文件中的参数失败：请检查目标文件中是否存在${l_paramName}参数"
          else
            warn "更新目标文件中的${l_paramName}参数失败"
          fi
        else
          info "目标文件中${l_paramName}参数合并成功，值为:${l_paramValue}"
        fi
      else
        l_unprocessedParams[${l_index}]="${l_paramName}"
        ((l_index = l_index + 1))
      fi
    done

    while [ "${l_index}" -gt 0 ];do
      l_unprocessedParamStr=""
      for ((l_i=0; l_i < l_index; l_i++));do
        l_paramName="${l_unprocessedParams[${l_i}]}"
        _combine "${l_srcYamlFile}" "${l_targetYamlFile}" "${l_paramName}" "${l_allowInsertNewListItem}" \
          "${l_exitOnFailure}" "${l_reserveDeleteItem}"
        if [ "${gDefaultRetVal}" ];then
          l_unprocessedParamStr="${l_unprocessedParamStr},${gDefaultRetVal}"
        fi
      done
      ((l_index = 0))
      if [ "${l_unprocessedParamStr}" ];then
        l_unprocessedParamStr="${l_unprocessedParamStr:1}"
        # shellcheck disable=SC2206
        l_unprocessedParams=(${l_unprocessedParamStr//,/ })
        l_index="${#l_unprocessedParams[@]}"
      else
        l_unprocessedParams=()
      fi
    done
  ...
  unset l_srcYamlFile
  unset l_targetYamlFile
  unset l_exitOnFailure
  ...
  unset l_paramName
  unset l_paramValue
  unset l_unprocessedParams
  unset l_unprocessedParamStr
}
...
#           l_paramArray=(${_cachedParams//,/ })
#           # shellcheck disable=SC2145
#           if [[ "${l_paramArray[4]}" == "true" ]];then
#             #缓存的数据中起止行号必须是参数下属的数据块的起止行号。
#             #首次新插入某个参数时，_cachedParams数据中的起始行号是该参数所在行的行号，
#             #因此首次新插入后进行缓存的参数，其中起始行号要加1，以指向其下数据块的起始行号。
#             ((l_paramArray[0] = l_paramArray[0] + 1))
#             # shellcheck disable=SC2124
#             _cachedParams="${l_paramArray[@]}"
#             _cachedParams="${_cachedParams// /,}"
#           fi
           echo "-22----_cachedParams= ${_cachedParams} ----- ${gDefaultRetVal} ---"
           #更新父级缓存数据:
           gFileDataBlockMap["${l_cachedParamKey}"]="${_cachedParams}"
         fi
         ;;
...
function __readOrWriteYamlFile() {
  ...
  #标识l_paramPath路径是否是上一层递归调用插入的新路径。
  local l_isNewInsertPath=${10}
  ...
  if [ "${l_content}" ];then
    ...
    if [[ "${l_initStartRowNum}" -eq "${l_initEndRowNum}" && ${l_isNewInsertPath} == "true" ]];then
      #起始行号等于截止行号的情况下，l_initStartRowNum是参数所在行，需要在其下层数据块中查找目标参数，
      #因此起始行的前导空格数还要在l_tmpSpaceNum1的基础上加2.
  ...
  if [ "${l_startRowNum}" -lt 0 ];then
    ...
    case ${l_mode} in
      ...
      "insert")
        #插入模式下：自动创建缺失的参数路径。
        _insertParamDirectly "${l_yamlFile}" "${l_stdParamName}" "${l_initStartRowNum}" \
          "${l_initEndRowNum}" "${l_lastArrayIndex}"
        # shellcheck disable=SC2206
        l_rowNumAndSpaceNum=(${gDefaultRetVal})
        #设置定位目标参数行的位置和前导空格数量。
        l_startRowNum="${l_rowNumAndSpaceNum[0]}"
  ...
  #修正缓存数据。
#  if [[ "${l_isInsertParam}" == "true" ]];then
#    #缓存的数据中起止行号必须是参数下属的数据块的起止行号。
#    #首次新插入某个参数时，_cachedParams数据中的起始行号是该参数所在行的行号，
#    #因此首次新插入后进行缓存的参数，其中起始行号要加1，以指向其下数据块的起始行号。
#    # shellcheck disable=SC2206
#    l_pathArray=(${_cachedParams//,/ })
#    ((l_pathArray[0] = l_pathArray[0] + 1))
#    # shellcheck disable=SC2124
#    _cachedParams="${l_pathArray[@]}"
#    _cachedParams="${_cachedParams// /,}"
#  fi
  #向缓存数据追加l_isInsertParam参数。
  _cachedParams="${_cachedParams},${l_isInsertParam}"
    ...
    if [ ! "${l_retValue}" ];then
      if [[ "${l_paramPath}" =~ ^(.*)\.(.*)$ ]];then
        ...
        __readOrWriteYamlFile "${l_mode}" "${l_yamlFile}" "${l_paramPath}" "${l_paramValue}" \
          "${l_rowNum}" "${l_endRowNum}" "${l_keepOriginalFormat}" "${l_lastArrayIndex}" "${l_isBlockRow}" "${l_isInsertParam}"
        l_retValue="${gDefaultRetVal}"
      else
        #根据l_mode参数的值进行后续处理。
        case ${l_mode} in
...
function _getReadContent() {
  ...
  if [ "${l_lineCount}" -eq  1 ];then
    ...
    if [[ "${l_isSampleValue}" == "false" || "${l_keepOriginalFormat}" == "false" && "${l_arrayIndex}" -ge 0 && "${l_content}" =~ ^([ ]*)\- ]];then
      l_content="${l_content/-/ }"
      l_tmpSpaceNum=$(echo -e "${l_content}" | grep -o "^[ ]*" | grep -o " " | wc -l)
...
function _createCacheForParamPath(){
  ...
  l_mapKey="${l_mapKey//\[/\\\[}"
  l_mapKey="${l_mapKey//\]/\\\]}"
  gFileDataBlockMap["${l_mapKey}"]="${l_startRowNum},${l_endRowNum},${l_index},false"
...
function _combine(){
  ...
  local l_insertMode
  local l_index
  local l_tmpParamValue

  l_unprocessedParamStr=""

  ((l_targetIndex = -1))
  l_targetParamPath="${l_paramPath}"
  l_insertMode="false"

  l_paramName=$(echo -e "${l_paramPath}" | grep -oP "^(.*)\[\d+\]$")
  if [ "${l_paramName}" ];then
    #是列表项，则需要判断是简单值列表项，还是对象型列表项？
    #如果是对象型列表项，需要根据列表项的name属性的值，动态查找目标文件中对应name值的列表项的序号。
    #向目标文件写入时需要使用这个序号；
    #如果没有name属性或属性值为空，则:
    #  如果exitOnFailure为true，则直接报错退出，
    #  如果exitOnFailure为false，则输出告警信息后，直接使用源路径中的序号。
    readParam "${l_srcYamlFile}" "${l_paramPath}.name"
    if [[ ! "${gDefaultRetVal}" ||  "${gDefaultRetVal}" == "null" ]];then
      if [[ "${l_exitOnFailure}" && "${l_exitOnFailure}" == "true" ]];then
        error "源文件${l_srcYamlFile##*/}中${l_paramPath}.name参数异常：不存在或为空。"
      else
        warn "源文件${l_srcYamlFile##*/}中${l_paramPath}.name参数异常：不存在或为空。"
        warn "合并${l_paramPath}参数时将采用\"按列表项序号进行合并\"的策略。"
      fi
    else
      #查找目标文件中同名列表项的序号。
      findListItemIndex "${l_targetYamlFile}" "${l_paramPath%[*}" "name" "${gDefaultRetVal}"
      # shellcheck disable=SC2206
      l_params=(${gDefaultRetVal})
      if [ "${l_params[0]}" -ne -1 ];then
        l_targetIndex="${l_params[0]}"
      elif [ "${l_allowInsertNewListItem}" == "true" ];then
        #直接设置索引序号为该列表项的总项数,这样即可将新增的列表项直接插入到目标文件列表项的末尾。
        l_targetIndex="${l_params[1]}"
        l_insertMode="true"
      fi
    fi

    if [ "${l_targetIndex}" -ne -1 ];then
      l_targetParamPath="${l_paramPath%[*}[${l_targetIndex}]"
    fi
  fi

  readParam "${l_srcYamlFile}" "${l_paramPath}"
  l_paramValue="${gDefaultRetVal}"

  if [ ! "${l_paramValue}" ];then
    updateParam "${l_targetYamlFile}" "${l_targetParamPath}" ""
  elif [ "${l_paramValue}" == "null" ];then
    if [[ "${l_exitOnFailure}" && "${l_exitOnFailure}" == "true" ]];then
      error "读取源文件中${l_paramPath}参数失败：请检查源文件中是否存在该参数"
    else
      warn "读取源文件中${l_paramPath}参数失败"
    fi
  elif [ "${l_insertMode}" == "true" ];then
    #插入整个参数数据块。
    insertParam "${l_targetYamlFile}" "${l_targetParamPath}" "${l_paramValue}"
    info "向${l_targetYamlFile##*/}文件中新增参数${l_targetParamPath}，其值为：${l_paramValue}"
  else
    #检查参数值是否是列表类型
    l_srcContent=$(echo -e "${l_paramValue}" | grep -oP "^(\-)(.*)$")
    if [ "${l_srcContent}" ];then
      #继续判断：如果是数组型列表，则直接全参替换。
      #否则逐项按name属性值，或列表序号（没有定义name属性时）进行匹配，匹配到后逐个属性进行替换。
      getListTypeByContent "${l_srcContent}"
      if [ "${gDefaultRetVal}" == "array" ];then
        #是数组，则直接整体替换。
        updateParam "${l_targetYamlFile}" "${l_targetParamPath}" "${l_paramValue}"
        info "检测到数组型列表参数${l_targetParamPath}，整体替换列表参数值为:\n${l_paramValue}"
      else
        l_tmpParamValue=$(echo -e "${l_srcContent}" | grep -noP "^([ ]*)- \![ ]*$")
        if [ "${l_tmpParamValue}" ];then
          #如果存在"- !"项，则将"- !"项删除后直接整体替换数组参数。
          if [ "${l_reserveDeleteItem}" == "false" ];then
            l_tmpParamValue="${l_tmpParamValue%%:*}"
            l_paramValue=$(echo -e "${l_paramValue}" | sed "${l_tmpParamValue}d")
          fi
          updateParam "${l_targetYamlFile}" "${l_targetParamPath}" "${l_paramValue}"
          info "未在${l_targetParamPath}列表参数中检测到\"- !\"项，整体替换列表参数值为:\n${l_paramValue}"
        else
          stringToArray "${l_srcContent}" "l_params"
          l_paramCount="${#l_params[@]}"
          for ((l_i = 0; l_i < l_paramCount; l_i++));do
            if [[ "${l_params[${l_i}]}" =~ ^(\-)([ ]*)$ ]];then
              warn "检测到源文件中${l_paramPath}[${l_i}]参数没有配置,则跳过目标文件中${l_paramPath}[${l_i}]的参数不进行合并。"
            else
              l_unprocessedParamStr="${l_unprocessedParamStr},${l_paramPath}[${l_i}]"
            fi
          done
        fi
      fi
    else
      l_srcContent=$(echo -e "${gDefaultRetVal}" | grep -oP "^[a-zA-Z_]+(.*):(.*)$")
      if [ ! "${l_srcContent}" ];then
        l_paramValue="${gDefaultRetVal}"
        ...
        if [[ "${gDefaultRetVal}" =~ ^(\-1) ]];then
          if [[ "${l_exitOnFailure}" && "${l_exitOnFailure}" == "true" ]];then
            error "更新目标文件中的参数失败：请检查目标文件中是否存在${l_paramPath}参数"
          else
            warn "更新目标文件中的${l_paramPath}参数失败"
          fi
        else
          info "目标文件中${l_paramPath}参数合并成功，值为:${l_paramValue}"
        fi
      else
        stringToArray "${l_srcContent}" "l_params"
        l_paramCount="${#l_params[@]}"
        for ((l_i = 0; l_i < l_paramCount; l_i++));do
          ...
          if [ "${l_targetParamPath}" ];then
            l_paramName="${l_targetParamPath}.${l_paramName}"
          fi
          l_paramValue="${l_params[${l_i}]#*:}"
          if [[ "${l_paramValue}" && ! "${l_paramValue}" =~ ^([ ]*)$ ]];then
            ...
            l_unprocessedParamStr="${l_unprocessedParamStr},${l_paramName}"
          fi
        done
      fi
  ...
  gDefaultRetVal="${l_unprocessedParamStr:1}"
