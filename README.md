...
6. istio
    ...
    另外，wydevops会连接目标集群(由目标参数targetApiServer指定)动态获取所有生成的K8S资源类型的apiVersion参数，
...
2. 定义环境变量WYDEVOPS_HOME,指向下载的项目根目录。
   ubuntu下依次执行：
   1）vim ~/.bashrc
   2）在文件末尾添加：export WYDEVOPS_HOME={下载的项目根目录}, 并保存退出。
   ...
   5）变量值 输入您下载的项目根目录。
   6）点击确定保存。您需要重新打开一个新的终端窗口来使设置生效。
3. 安装第三方的依赖库（安装方法详见前述）
4. 安装成功验证
   执行命令：bash $WYDEVOPS_HOME/script/wydevops.sh -h
   如果没有报错，说明安装成功。

## 与需要打包部署的项目集成
1. 将$WYDEVOPS_HOME/script/wydevops-run.sh文件复制到目标项目的根目录下。
2. 打开目标项目的根目录下wydevops-run.sh文件，在其末尾的执行wydevops.sh命令的参数行中做如下修改或确认：
   ...
   5) 其他参数保持不变即可，如需修改可执行命令：bash $WYDEVOPS_HOME/script/wydevops.sh -h，进行参数详情查询。 
3. 在目标项目的根目录下创建一个名为ci-cd-config.yaml的文件。 
   ...
   请参考$WYDEVOPS_HOME/script/templates/config目录下各语言的配置模板文件_ci-cd-template.yaml。该文件中包含了所有的配置参数详情。
...
2. go项目默认规范
   ...
   3) app.gateway.domain——该参数值被绑定到了globalParams.gatewayHost。      
   4) app.gateway.route-prefix——该参数值被绑定到了globalParams.gatewayPath       
   5) app.port——该参数值被绑定到了globalParams.mainPort、globalParams.containerPorts、globalParams.servicePorts 
   6) app.version——该参数值被绑定到了globalParams.businessVersion。  
